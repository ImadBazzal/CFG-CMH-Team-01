...
const stateCoordinates = {
  ...
  'MO': [38.456085, -92.288368], 'MT': [47.052952, -110.454353], 'NE': [41.125370, -98.268082], 'NV': [38.313515, -117.055374],
...
export function getCoordinates(city, state, index = 0) {
  // Get state coordinates
  if (stateCoordinates[state]) {
    const [lat, lng] = stateCoordinates[state];
    // Add deterministic offset based on index to avoid overlapping
    const offsetLat = (index * 0.3) % 2 - 1;
    const offsetLng = (index * 0.4) % 2 - 1;
    return [lat + offsetLat, lng + offsetLng];
  }
  
  // Default to center of US with offset
  return [39.8283 + (index * 0.1) % 2 - 1, -98.5795 + (index * 0.1) % 2 - 1];
}